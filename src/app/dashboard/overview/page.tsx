...
function randomizeMetrics() {
  return [
    {
      label: "Revenue",
      value: `$${getRandomInt(35000, 55000).toLocaleString()}`,
      change: `+${getRandomInt(5, 15)}%`,
      color: "text-green-600",
    },
    {
      label: "Users",
      value: getRandomInt(7000, 12000).toLocaleString(),
      change: `+${getRandomInt(2, 8)}%`,
      color: "text-blue-600",
    },
    {
      label: "Conversions",
      value: getRandomInt(900, 1600).toLocaleString(),
      change: `+${getRandomInt(1, 10)}%`,
...
function randomizeBarChartSeries() {
  return ALL_CAMPAIGNS.flatMap((name) =>
    Array.from({ length: 30 }, (_, i) => {
      const d = new Date(Date.now() - i * 24 * 60 * 60 * 1000);
      return {
        name,
        date: d.toISOString().slice(0, 10),
        value: getRandomInt(400, 2000),
      };
    })
  );
}
...
function randomizePieChartSeries() {
  return ALL_SOURCES.flatMap((source) =>
    Array.from({ length: 30 }, (_, i) => {
      const d = new Date(Date.now() - i * 24 * 60 * 60 * 1000);
      return {
        name: source.name,
...
function randomizeTable() {
  return ALL_CAMPAIGNS.flatMap((campaign) =>
    Array.from({ length: 30 }, (_, i) => {
      const d = new Date(Date.now() - i * 24 * 60 * 60 * 1000);
...
export default function OverviewPage() {
  ...
  const [selectedSources, setSelectedSources] = useState<string[]>(
    ALL_SOURCES.map((s) => s.name),
  );
  const [loading, setLoading] = useState(true);
  const [metrics, setMetrics] = useState<
    null | { label: string; value: string; change: string; color: string }[]
  >(null);
  const [lineData, setLineData] = useState<
    null | { name: string; value: number; date: string }[] | null
  >(null);
  const [barSeries, setBarSeries] = useState<
    null | { name: string; value: number; date: string }[] | null
  >(null);
  const [pieSeries, setPieSeries] = useState<
    null | { name: string; value: number; color: string; date: string }[] | null
  >(null);
  ...
  const filteredBarData = ALL_CAMPAIGNS.filter((name) =>
    selectedCampaigns.includes(name),
  ).map((name) => ({
    name,
    value: barSeries
      ? barSeries
          .filter((d) => d.name === name && isInRange(d.date))
          .reduce((sum, d) => sum + d.value, 0)
      : 0,
  }));

  const filteredPieData = ALL_SOURCES.filter((source) =>
    ...
    value: pieSeries
      ? pieSeries
          .filter((d) => d.name === source.name && isInRange(d.date))
          .reduce((sum, d) => sum + d.value, 0)
      : 0,
  }));

  const filteredTable = ALL_CAMPAIGNS.filter((campaign) =>
    selectedCampaigns.includes(campaign),
  ).map((campaign) => {
    const rows = table
      ? table.filter((d) => d.campaign === campaign && isInRange(d.date))
      : [];
    if (rows.length === 0) {
      const impressions = getRandomInt(100, 500);
      const clicks = getRandomInt(
        Math.floor(impressions * 0.05),
        Math.floor(impressions * 0.2),
      );
      const conversions = getRandomInt(
        Math.floor(clicks * 0.05),
        Math.max(1, Math.floor(clicks * 0.2)),
      );
      const ctr = ((clicks / impressions) * 100).toFixed(1) + "%";
      const cvr = ((conversions / clicks) * 100).toFixed(1) + "%";
      return {
    ...
    const ctr =
      (
        rows.reduce((sum, d) => sum + parseFloat(d.ctr), 0) / rows.length
      ).toFixed(1) + "%";
    const cvr =
      (
        rows.reduce((sum, d) => sum + parseFloat(d.cvr), 0) / rows.length
      ).toFixed(1) + "%";
      ...
      {loading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-black/10">
          <div className="flex flex-col items-center">
            <RunningAnimal />
            <div className="mt-4 text-lg font-semibold text-yellow-700 drop-shadow animate-pulse">
              Loading insights…
            </div>
          </div>
        </div>
          ...
          {loading || !metrics
            ? Array(4)
                ...
                .map((_, i) => (
                  <Card
                    key={i}
                    className="flex flex-col gap-2 items-start justify-center min-h-[120px]"
                  >
            ...
            : metrics.map(
                (metric: {
                  ...
                  <Card
                    key={metric.label}
                    className="flex flex-col gap-2 items-start justify-center min-h-[120px] transition-all duration-500"
                  >
